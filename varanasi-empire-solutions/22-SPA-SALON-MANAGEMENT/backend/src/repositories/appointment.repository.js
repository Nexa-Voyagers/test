...
export const appointment_Repository = {
  async create(data) {
    ...
    const query = \`INSERT INTO appointment_s (\${keys.join(',')}) VALUES (\${placeholders}) RETURNING *\`;
    const result = await pool.query(query, values);
    return result.rows[0];
  },
  async findAll(filters = {}) {
    const query = 'SELECT * FROM appointment_s ORDER BY created_at DESC LIMIT \$1 OFFSET \$2';
    const result = await pool.query(query, [filters.limit || 50, filters.offset || 0]);
    return result.rows;
  },
  async findById(id) {
    const result = await pool.query('SELECT * FROM appointment_s WHERE id = \$1', [id]);
  ...
  async update(id, data) {
    ...
    const query = \`UPDATE appointment_s SET \${fields.join(', ')} WHERE id = \$\${paramCount} RETURNING *\`;
    const result = await pool.query(query, values);
    if (result.rows.length === 0) throw new NotFoundError('appointment not found');
    return result.rows[0];
  },
  async delete(id) {
    const result = await pool.query('DELETE FROM appointment_s WHERE id = \$1 RETURNING *', [id]);
    if (result.rows.length === 0) throw new NotFoundError('appointment not found');
    return result.rows[0];
  }
};
