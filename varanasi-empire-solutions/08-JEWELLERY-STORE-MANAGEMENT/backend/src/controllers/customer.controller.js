...
export const getAll = asyncHandler(async (req, res) => {
  const result = await '${controller}'Service.getAll(req.user.storeId, req.query);
...
export const create = asyncHandler(async (req, res) => {
  const result = await '${controller}'Service.create({ ...req.body, storeId: req.user.storeId, createdBy: req.user.id });
