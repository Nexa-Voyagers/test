...
apiRouter.use('/hotels', authMiddleware, hotelRoutes);
apiRouter.use('/rooms', authMiddleware, roomRoutes);
apiRouter.use('/bookings', authMiddleware, bookingRoutes);
apiRouter.use('/guests', authMiddleware, guestRoutes);
apiRouter.use('/housekeeping', authMiddleware, housekeepingRoutes);
...
app.get('/', (req, res) => {
  res.json({
    success: true,
    message: 'Hotel & Hospitality Management System API',
    version: API_VERSION,
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`🚀 Hotel Management System running in ${process.env.NODE_ENV} mode on port ${PORT}`);
