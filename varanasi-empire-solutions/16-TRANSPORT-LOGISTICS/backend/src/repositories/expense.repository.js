...
export const expenseRepository = {
  async create(data) {
    ...
    const query = \`INSERT INTO expenses (${keys.join(',')}) VALUES (${placeholders}) RETURNING *\`;
    const result = await pool.query(query, values);
    return result.rows[0];
  },
  async findAll(filters = {}) {
    const query = 'SELECT * FROM expenses ORDER BY created_at DESC LIMIT $1 OFFSET $2';
    const result = await pool.query(query, [filters.limit || 50, filters.offset || 0]);
    return result.rows;
  },
  async findById(id) {
    const result = await pool.query('SELECT * FROM expenses WHERE id = $1', [id]);
    if (result.rows.length === 0) throw new NotFoundError('expense not found');
    return result.rows[0];
  },
